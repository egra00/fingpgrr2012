...
import be.ac.ucl.ingi.totem.scenario.model.jaxb.impl.CBGPRunImpl;
 ...
 */
public class CBGPRun extends CBGPRunImpl implements Event {
    
    private static final Logger logger = Logger.getLogger(CBGPRun.class);
    
    public EventResult action() throws EventExecutionException {
        try {
            CBGPSimulator cbgp = (CBGP) RepositoryManager.getInstance().getAlgo("CBGP");
            /*
             String sStopAt= this.getStopAt();
             if (sStopAt != null) {
             try {
             Integer iStopAt = Integer.decode(sStopAt);
             } catch (Exception e) {
             e.printStackTrace();
             }
             System.out.println("sim StopAt " + sStopAt);
             //cbgp.executeCommand("sim stop at " + sStopAt);
              }*/
            try {
                cbgp.simRun();
