...
import be.ac.ucl.ingi.totem.scenario.model.jaxb.impl.CBGPExecuteImpl;
 ...
 * --------
 */
public class CBGPExecute extends CBGPExecuteImpl implements Event {
    
    private static final Logger logger = Logger.getLogger(CBGPExecute.class);
    
    public EventResult action() throws EventExecutionException {
        try {
            CBGPSimulator cbgp = (CBGP) RepositoryManager.getInstance().getAlgo("CBGP");
            try {
                cbgp.runCmd(this.getCommand());
            } catch (RoutingException e) {
                System.out.println("CBGPExecute command failed");
        ...
        logger.info("CBGP command \""+ this.getCommand() + "\" executed.");
