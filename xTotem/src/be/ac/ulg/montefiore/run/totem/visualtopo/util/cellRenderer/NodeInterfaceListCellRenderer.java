...
*/
public class NodeInterfaceListCellRenderer extends DefaultListCellRenderer {
    public Component getListCellRendererComponent(JList list, Object value, int index, boolean isSelected, boolean cellHasFocus) {
        super.getListCellRendererComponent(list, value, index, isSelected, cellHasFocus);
        if (value instanceof NodeInterface) {
            NodeInterface nif = (NodeInterface) value;
            if (nif.isSetIp() && nif.getIp().isSetValue())
                setText(nif.getId() + " (" + nif.getIp().getValue() + ")");
            else setText(nif.getId());
        } else {
