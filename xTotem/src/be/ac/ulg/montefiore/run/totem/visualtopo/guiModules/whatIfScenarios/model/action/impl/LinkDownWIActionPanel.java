...
/**
* Panel to select a link to set down.
*
...
public class LinkDownWIActionPanel extends WIActionPanel {
    private JComboBox linksCmb;
    private JCheckBox reverseLinkChk;

    public LinkDownWIActionPanel(Domain domain) {
        super(new GridLayout(3, 1, 5, 5));
        this.domain = domain;
        add(new JLabel("Select the link to set down."));
        ...
        setBorder(BorderFactory.createTitledBorder("Link Down"));
    }

    public WIAction createWIAction() throws BadParametersException {
        WIAction action;
        try {
            action = new LinkDownWIAction(domain, (Link)linksCmb.getSelectedItem(), reverseLinkChk.isSelected());
