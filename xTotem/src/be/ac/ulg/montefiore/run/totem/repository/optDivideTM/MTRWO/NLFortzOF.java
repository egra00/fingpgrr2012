 ...
 */
public class NLFortzOF implements TEObjectiveFunction {
    private static Logger logger = Logger.getLogger(NLFortzOF.class);

    public float getFirstDerivate(float capacity, double load) {
        return (float) ((capacity * capacity) / ((capacity - load) * (capacity - load)));
    }
    ...
    public double getScore(double linkCapacities[], double linkLoads[]) {
        ...
        for (int i=0; i < linkLoads.length; i++) {
            score += (linkLoads[i] / (1 - (linkLoads[i] / linkCapacities[i])));
        }
    ...
    public String getName() {
        return new String("NonLinearFortz");
    }
