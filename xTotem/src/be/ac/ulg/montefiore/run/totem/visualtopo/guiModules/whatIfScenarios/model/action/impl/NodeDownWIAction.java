...
public class NodeDownWIAction implements WIAction {

    private static final Logger logger = Logger.getLogger(NodeDownWIAction.class);

    private Node node = null;
    private Domain domain = null;

    public NodeDownWIAction(Domain domain, Node node) throws NodeNotFoundException {
        this.domain = domain;
        setNode(node);
    }
    ...
    public void execute() throws ActionExecutionException {
        if (node.getNodeStatus() == Node.STATUS_DOWN) {
            logger.warn("Trying to set down a node that is already down.");
        }
