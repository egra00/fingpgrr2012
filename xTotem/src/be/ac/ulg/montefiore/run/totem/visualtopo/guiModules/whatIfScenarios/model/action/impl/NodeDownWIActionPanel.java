...
/**
* Panel to select a node to set down.
*
...
public class NodeDownWIActionPanel extends WIActionPanel {
    private JComboBox nodesCmb;

    public NodeDownWIActionPanel(Domain domain) {
        super(new GridLayout(2, 1, 5, 5));
        this.domain = domain;
        add(new JLabel("Select the node to set down."));
        ...
        setBorder(BorderFactory.createTitledBorder("Node Down"));
    }

    public WIAction createWIAction() throws BadParametersException {
        WIAction action;
        try {
            action = new NodeDownWIAction(domain, (Node)nodesCmb.getSelectedItem());
