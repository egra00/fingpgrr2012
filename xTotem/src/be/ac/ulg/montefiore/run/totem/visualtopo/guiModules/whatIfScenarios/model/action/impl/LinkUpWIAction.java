...
public class LinkUpWIAction implements WIAction {
    private static final Logger logger = Logger.getLogger(LinkUpWIAction.class);
    
    private Domain domain;
    private Link link;
    private boolean reverseLink;

    public LinkUpWIAction(Domain domain, Link link) throws LinkNotFoundException {
        this(domain, link, false);
    }

    public LinkUpWIAction(Domain domain, Link link, boolean reverseLink) throws LinkNotFoundException {
        this.domain = domain;
    ...
    private int setLinkUp(Link l) throws StatusTypeException {
        int oldStatus = l.getLinkStatus();

        if (oldStatus == Link.STATUS_UP) {
            logger.warn("Trying to set up a link that is already up.");
        }

        l.setLinkStatus(Link.STATUS_UP);

        return oldStatus;
    }

    public String getName() {
        return "Link up: " + link.getId();
    }
}
