 ...
 */
public class WMeanDelayOF implements TEObjectiveFunction {
    private static Logger logger = Logger.getLogger(WMeanDelayOF.class);

    public float getFirstDerivate(float capacity, double load) {
        return (float) (capacity / ((capacity - load) * (capacity - load)));
    }
    ...
    public double getScore(double linkCapacities[], double linkLoads[]) {
        ...
        for (int i=0; i < linkLoads.length; i++) {
            score += (linkLoads[i] / (linkCapacities[i] - linkLoads[i]));
        }
