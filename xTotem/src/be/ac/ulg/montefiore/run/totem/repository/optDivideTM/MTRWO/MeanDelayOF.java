 ...
 */
public class MeanDelayOF implements TEObjectiveFunction {
    private static Logger logger = Logger.getLogger(MeanDelayOF.class);

    public float getFirstDerivate(float capacity, double load) {
        if (capacity > load) {
            return (float) (1 / ((capacity - load) * (capacity - load)));
        } else {
    ...
    public double getScore(double linkCapacities[], double linkLoads[]) {
        ...
        for (int i=0; i < linkLoads.length; i++) {
            if (linkCapacities[i] > linkLoads[i]) {
                score += (1 / (linkCapacities[i] - linkLoads[i]));
            } else {
