...
var y {u in VERTICES, v in VERTICES, l in LINKS} binary;
...
subject to primaryFlowConservationC{u in VERTICES, v in VERTICES, n in VERTICES}:
    rho[u,n] + (sum{l in LINKS} x[u,v,l] * InLinks[n,l]) = rho[v,n] + (sum{l in LINKS} x[u,v,l] * OutLinks[n,l]);

subject to backupFlowConservationC{u in VERTICES, v in VERTICES, n in VERTICES}:
    rho[u,n] + (sum{l in LINKS} y[u,v,l] * InLinks[n,l]) = rho[v,n] + (sum{l in LINKS} y[u,v,l] * OutLinks[n,l]);

subject to disjointNessC{u in VERTICES, v in VERTICES, l in LINKS}:
    x[u,v,l] + y[u,v,l] <= 1;

subject to utilizationC{l in LINKS}:
    utilization[l] = ((sum{u in VERTICES, v in VERTICES} (x[u,v,l] + y[u,v,l]) * Demand[u,v]) / Capa[l]);
